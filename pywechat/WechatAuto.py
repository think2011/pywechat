...
from .WechatTools import Tools,mouse,Desktop,FindWindow,IsIconic,keyboard
...
class Messages():
    ...
    def forward_message(friends:list[str],message:str,delay:float=0.2,search_pages:int=5,wechat_path:str=None,is_maximize:bool=True,close_wechat:bool=True):
        ...
        def right_click_message():
            ...
            menu=main_window.child_window(title='',class_name='MenuWnd',control_type='Menu',framework_id='Win32')
    ...
    def check_new_message(duration:str=None,wechat_path:str=None,close_wechat:bool=True):
        ...
        Returns:
            ...
            json格式:[{'名称':'好友备注','新消息条数',\d+:'类型':'群聊or个人or公众号','消息':[str]}]\n
        '''
        Names=[]#有新消息的好友名称
        ...
        def get_message_content(name,number,search_pages):
            ...
            if voice_call_button.exists() and not video_call_button.exists():#只有语音和聊天按钮没有视频聊天按钮是群聊
                ...
                Messages.extend([message for message in messages if not re.findall(r'\d\d:\d\d',message.window_text())])
                main_window.click_input()
                while len(list(set(Messages)))<number:
                    ...
                    messages=chatList.children(control_type='ListItem')
                    Messages.extend([message for message in messages if not re.findall(r'\d\d:\d\d',message.window_text())])
                Messages=[message.window_text() for message in Messages]
            ...
            if video_call_button.exists() and video_call_button.exists():#同时有语音聊天和视频聊天按钮的是个人
                ...
                main_window.click_input()
                while len(list(set(Messages)))<number:
                    if check_more_messages_button.exists():#滑动到顶部出现查看新消息标识的按钮,点击后继续向上。
                        check_more_messages_button.click_input()
                        main_window.click_input()
                    pyautogui.press('pageup',_pause=False)
                    messages=chatList.children(control_type='ListItem')
                    Messages.extend([message for message in messages if not re.findall(r'\d\d:\d\d',message.window_text())])
                Messages=[message.window_text() for message in Messages]
                Messages=Messages[-number:]#从后往前倒数number条消息
                pyautogui.press('End')
                return {'名称':name,'新消息条数':number,'类型':type,'消息':Messages}
            else:#都没有是公众号
                type='公众号'
