...
class Messages():
    @staticmethod
    def send_message_to_friend(friend:str,message:str,delay:float=0.2,tickle:bool=False,search_pages:int=5,wechat_path:str=None,is_maximize:bool=True,close_wechat:bool=True):
        '''
        该方法用于给单个好友或群聊发送单条信息\n
        Args:
    ...
    @staticmethod
    def send_messages_to_friend(friend:str,messages:list[str],tickle:bool=False,delay:float=0.4,search_pages:int=5,wechat_path:str=None,is_maximize:bool=True,close_wechat:bool=True):
        '''
        该方法用于给单个好友或群聊发送多条信息\n
        Args:
            friend:\t好友或群聊备注。格式:friend="好友或群聊备注"\n
    ...
    @staticmethod
    def send_messages_to_friends(friends:list[str],messages:list[list[str]],tickle:list[bool]=[],delay:float=0.4,wechat_path:str=None,is_maximize:bool=True,close_wechat:bool=True):
        '''
        该方法用于给多个好友或群聊发送多条信息\n
        Args:
    ...
    @staticmethod
    def send_message_to_friends(friends:list[str],message:list[str],tickle:list[bool]=[],delay:float=0.4,wechat_path:str=None,is_maximize:bool=True,close_wechat:bool=True):
        '''
...
class AutoReply():
    ...
    @staticmethod
    def auto_reply_messages(content:str,duration:str,dontReplytoGroup:bool=False,max_pages:int=5,never_reply:list=[],scroll_delay:int=0,wechat_path:str=None,is_maximize:bool=True,close_wechat:bool=True):
        '''
    ...
    def auto_reply_to_group(group_name:str,duration:str,content:str,at_only:bool=True,maxReply:int=3,at_others:bool=True,save_chat_history:bool=False,capture_screen:bool=False,folder_path:str=None,search_pages:int=5,wechat_path:str=None,is_maximize:bool=True,close_wechat:bool=True):
        ...
        Args:
            ...
            maxReply:\t最多同时回复群内连续发送的n条新消息,默认为5,不用设置特别大\n
            save_chat_history:\t是否保存自动回复时留下的聊天记录,若值为True该函数返回值为聊天记录json,否则该函数无返回值。\n
            capture_screen:\t是否保存聊天记录截图,默认值为False不保存。\n
            search_pages:\t在会话列表中查询查找好友时滚动列表的次数,默认为5,一次可查询5-12人,当search_pages为0时,直接从顶部搜索栏搜索好友信息打开聊天界面\n
            folder_path:\t存放聊天记录截屏图片的文件夹路径\n
        ...
        def at_others(who):
            chat.click_input()
            chat.type_keys(f'@{who}')
            pyautogui.press('enter',_pause=False)
        def send_message(newMessage,who):
            if at_only:
                if who!=myname:
                    #at_only设置为True时需要判断我是否被@了
                    if f'@{myalias}' in newMessage:#如果消息中有@我的字样,那么回复
                        if at_others:
                            at_others(who)
                        pyautogui.hotkey('ctrl','v',_pause=False)
                        pyautogui.hotkey('alt','s',_pause=False)
        ...
        chat,main_window=Tools.open_dialog_window(friend=group_name,wechat_path=wechat_path,is_maximize=is_maximize,search_pages=search_pages)
        ...
        group_settings_window.child_window(**Buttons.MyNicknameInGroupButton).click_input() 
        ...
        while True:
            if time.time()-start_time<duration:
                ...
                for newMessage in filtered_newMessages:
                    message_sender,message_content,message_type=Tools.parse_message_content(ListItem=newMessage,friendtype='群聊',myname=myname)
...
def send_message_to_friends(friends:list[str],message:list[str],tickle:list[bool]=[],delay:float=0.2,wechat_path:str=None,is_maximize:bool=True,close_wechat:bool=True):
    '''
...
def listen_on_chat(friend:str,duration:str,save_file:bool=True,folder_path:str=None,search_pages:int=5,wechat_path:str=None,is_maximize:bool=True,close_wechat:bool=True)->tuple[list[str],list[str],list[str]]:
    '''
    该函数用来持续监听某个聊天窗口内的消息,如果有新文件则将其保存到制定路径。\n
    Args:
        friend:\t好友或群聊备注\n
        duration:\t自动回复持续时长,格式:'s','min','h'单位:s/秒,min/分,h/小时\n
        save_file:\t是否保存聊天记录中的文件,默认保存\n
        folder_path:\t保存聊天记录的文件夹路径,如果不传入则保存在当前运行代码所在的文件夹下\n
        search_pages:\t在会话列表中查询查找好友时滚动列表的次数,默认为5,一次可查询5-12人,当search_pages为0时,直接从顶部搜索栏法搜索好友信息打开聊天界面\n
        wechat_path:\t微信的WeChat.exe文件地址,主要针对未登录情况而言,一般而言不需要传入该参数,因为pywechat会通过查询环境变量,注册表等一些方法\n
            尽可能地自动找到微信路径,然后实现无论PC微信是否启动都可以实现自动化操作,除非你的微信路径手动修改过,发生了变动的话可能需要\n
    ...
    if not duration:#不按照指定的时间格式输入,需要提前中断退出
        raise TimeNotCorrectError
    if not folder_path and save_file:
    ...
    Systemsettings.close_listening_mode(volume=False)
    start_time=time.time()  
    while True:
        if time.time()-start_time<duration:
            ...
            if type=='好友':
                for newMessage in newMessages:
                    if newMessage not in recorded:
                        message_sender,message_content,message_type=Tools.parse_message_content(ListItem=newMessage,friendtype=type,myname=myname)
    ...
    if save_file and filelist:
        Systemsettings.copy_files(filelist,target_folder=folder_path)
        print(f'监听{unchanged_duration}内,已自动保存文件{len(filelist)}个')
    Systemsettings.close_listening_mode()
    if close_wechat:
        main_window.close()
    return message_content,message_senders,message_types
...
def at(group_name:str,group_member:str,is_send:bool=False,search_pages:int=5,wechat_path:str=None,is_maximize:bool=True,close_wechat:bool=True):
    '''
    ...
    Examples:
        ...
        ```
    '''
    editarea,main_window=Tools.open_dialog_window(friend=group_name,wechat_path=wechat_path,is_maximize=is_maximize,search_pages=search_pages)
    chat_history_button=main_window.child_window(**Buttons.ChatHistoryButton)
    ...
    #有聊天记录这个按钮,但是却没有视频和语音聊天这两个按钮,只可能是文件传输助手
    if not voice_call_button.exists() and not video_call_button.exists():
        main_window.close()
        raise NotFriendError(f'不能@文件传输助手!')
    # #同时有视频聊天和语音聊天按钮的是好友
    if voice_call_button.exists() and video_call_button.exists():
        friendtype='好友'
    #只有语音聊天按钮无视频聊天按钮的是群聊
    if voice_call_button.exists() and not video_call_button.exists():
        friendtype='群聊'
    if friendtype=='群聊':#只有群聊才能@
        Systemsettings.set_english_input()
        editarea.type_keys(f'@{group_member}')
        if not chatContactMenu.exists():#@后没有出现说明群聊里没这个人
            #按len(group_member)+1下backsapce把@xxx删掉
            pyautogui.press('backspace',presses=len(group_member)+1)
...
def at_all(group_name:str,is_send:bool=False,search_pages:int=5,wechat_path:str=None,is_maximize:bool=True,close_wechat:bool=True):
    '''
    ...
    if voice_call_button.exists() and not video_call_button.exists():
        friendtype='群聊'
    if friendtype=='群聊':#只有群聊才能@所有人
        editarea.type_keys(f'@')
...
def save_photos(friend:str,number:int,folder_path:str=os.getcwd(),is_maximize:bool=True,close_wechat:bool=True,search_pages:int=5,wechat_path:str=None):
    '''
    该函数用来保存与某个好友或群聊的图片到指定文件夹中,该函数目前不稳定,只能保存部分图片\n
    Args:
        friend:\t好友或群聊备注\n
        number:\t需要保存的图片数量,默认为10\n
     ...
     Examples:
        ...
        folder_path=r'E:\Desktop\新建文件夹'
        save_photos(friend='测试群',number=20,folder_path=folder_path)
        ```
    '''
    if folder_path and not Systemsettings.is_dirctory(folder_path):
        raise NotFolderError(f'所选路径不是文件夹!无法保存聊天记录,请重新选择!')
    Systemsettings.copy_text_to_windowsclipboard(folder_path)
    chat_history_window,main_window=Tools.open_chat_history(friend=friend,TabItem='图片与视频',close_wechat=close_wechat,is_maximize=is_maximize,wechat_path=wechat_path)
    activateScrollbarPosition=chat_history_window.rectangle().right-5,chat_history_window.rectangle().bottom-20
    photoList=chat_history_window.child_window(**Lists.PhotoAndVideoList)
    scroll_position=((photoList.rectangle().left+photoList.rectangle().right)//2,(photoList.rectangle().top+photoList.rectangle().bottom)//2)
    ...
    photoListItems=[ListItem for ListItem in photoList.children() if not ListItem.descendants(control_type='Text')]
    lastPhoto=photoListItems[-1]
    #先右键最后一个图片激活菜单
    lastPhoto.descendants(control_type='Button')[-1].right_click_input()
    #点击菜单里的多选按钮
    selectMenuItem.click_input()
    #点击后,会在下方出现一个白色的选项区域,为了防止误触,需要回到最底部
    mouse.click(coords=activateScrollbarPosition)
    pyautogui.press('end')
    selected_num=1
    if len(photoListItems)>=number:
        photoListItems=photoListItems[::-1][1:number+1]
        for photo in photoListItems:    
            photo.descendants(control_type='Button')[-1].click_input()
            selected_num+=1
            if selected_num!=1 and selected_num%4==0:#每点击选择四张就向上
                mouse.scroll(coords=scroll_position,wheel_dist=2)
    saveButton=chat_history_window.descendants(**Texts.SaveText)[0].parent().descendants(control_type='Button',title='')[0]
    saveButton.click_input()
    confirmWindow=chat_history_window.child_window(**Windows.FileSaveConfirmWindow)
    confirmButton=confirmWindow.child_window(**Buttons.ConfirmButton)
    confirmButton.click_input()
    Tools.NativeChooseFolder(folder_path)
    time.sleep(5)
    chat_history_window.close()

def forward_files(friend:str,others:list[str],number:int=10,is_maximize:bool=True,close_wechat:bool=True,search_pages:int=5,wechat_path:str=None):
    '''
...
def forward_links(friend:str,others:list[str],number:int=10,is_maximize:bool=True,close_wechat:bool=True,search_pages:int=5,wechat_path:str=None):
    '''
...
def forward_music_and_audio(friend:str,others:list[str],number:int=10,is_maximize:bool=True,close_wechat:bool=True,search_pages:int=5,wechat_path:str=None):
    '''
...
def forward_MiniPrograms(friend:str,others:list[str],number:int=10,is_maximize:bool=True,close_wechat:bool=True,search_pages:int=5,wechat_path:str=None):
    '''
