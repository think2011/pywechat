...
class Tools():
    ...
    def find_wechat_path(copy_to_clipboard:bool=True):
        '''该方法用来查找正在运行的微信的路径\n
        :param: copy_to_clipboard:\t是否将微信路径复制到剪贴板\n
        '''
        wechat_path=None
    ...
    def move_window_to_center(Window:dict=Main_window.MainWindow,Window_handle:int=0):
        ...
        :param: Window:\tpywinauto窗口类的kwargs参数
    ...
    def connect_to_wechat(wechat_path): 
        ...
        并且先使用win32GUI查找到窗口句柄后再传入pywinauto的connect方法,相较于open_wechat信息更稳定\n
        该方法用于连接到微信主界面,目前已弃用\n
        :param: wechat_path:\t微信路径
        '''
        #微信未打开，有登录选项
        max_retry_times=20
        retry_interval=0.4
                ...
                if win32gui.IsWindowVisible(main_window_handle):#主界面可见包含最小化
                    if IsIconic(main_window_handle):#主界面最小化
                        ...
                        try:#尝试通过窗口句柄查找
                            wechat=Application(backend='uia').connect(handle=main_window_handle)
                            main_window=wechat.window(**Main_window.MainWindow)
                        except ElementNotFoundError:#找不到再试试看使用进程号
    ...
    def open_dialog_window(friend:str,wechat_path:str=None,is_maximize:bool=True,search_pages:int=10): 
        '''
        该方法用于打开某个好友的聊天窗口\n
        :param: friend:\t好友或群聊备注名称,需提供完整名称\n
        :param: wechat_path:\t微信的WeChat.exe文件地址,当微信未登录时,该函数使用此地址启动微信并点击登录完成打开微信界面。\n
            这里强烈建议将微信路径加入到windows环境变量中,因为该函数默认使用windows环境变量中的Wechat.exe路径启动微信,此时调用该函数无需传入该参数
            若你没有设置微信的Wechat.exe地址为环境变量,那么你需要传入该文件地址作为参数否则会引发错误无法打开微信！
        :param: is_maximize:\t微信界面是否全屏,默认全屏。\n
        ...
        if search_pages:
            ...
            if is_maximize:
                main_window.maximize()
            if chat and chat.window_text()==friend:#chat不为None,即说明find_friend_in_MessageKist找到了聊天窗口chat,直接返回结果
    ...
    def find_friend_in_MessageList(friend:str,wechat_path:str=None,is_maximize:bool=True,search_pages:int=10):
        ...
        if current_chat.exists():#如果当前微信主界面右侧是聊天界面，
            if friend==current_chat.window_text():#如果当前主界面是某个好友的聊天界面且聊天界面顶部的名称为好友名称，直接返回结果
                chat=current_chat
                chat.click_input()
                return chat,main_window
            else:#否则在消息列表滚动中查找
    ...
    def open_friend_profile(friend:str,wechat_path:str=None,is_maximize:bool=True,search_pages:int=10):
        ...
        :param: wechat_path:\t微信的WeChat.exe文件地址,当微信未登录时,该函数使用此地址启动微信并点击登录完成打开微信界面。\n
            这里强烈建议将微信路径加入到windows环境变量中,因为该函数默认使用windows环境变量中的Wechat.exe路径启动微信,此时调用该函数无需传入该参数
            s若你没有设置微信的Wechat.exe地址为环境变量,那么你需要传入该文件地址作为参数否则会引发错误无法打开微信！
        :param: is_maximize:\t信界面是否全屏,默认全屏。\n
        :param: search_pages:\t在会话列表中查询查找好友时滚动列表的次数,默认为10,一次可查询5-12人,当search_pages为0时,直接从顶部搜索栏搜索好友信息打开聊天界面\n
        '''
        friend_settings_window,main_window=Tools.open_friend_settings(friend=friend,wechat_path=wechat_path,is_maximize=is_maximize,search_pages=search_pages)
        friend_button=friend_settings_window.child_window(title=friend,control_type="Button",found_index=0)
    ...
    @staticmethod
    def open_chat_history(friend:str,wechat_path:str=None,is_maximize:bool=True,close_wechat:bool=True,search_pages:int=10):
        '''
        该方法用于打开好友聊天记录界面\n
        :param: friend:\t好友备注名称,需提供完整名称\n
        :param: wechat_path:\t微信的WeChat.exe文件地址,当微信未登录时,该函数使用此地址启动微信并点击登录完成打开微信界面。\n
    ...
    def open_program_pane(wechat_path:str=None,is_maximize:bool=True,wechat_maximize:bool=True,close_wechat:bool=True):
        ...
        :param: wechat_path:\t微信的WeChat.exe文件地址,当微信未登录时,该函数使用此地址启动微信并点击登录完成打开微信界面。\n
            这里强烈建议将微信路径加入到windows环境变量中,因为该函数默认使用windows环境变量中的Wechat.exe路径启动微信,此时调用该函数无需传入该参数
            若你没有设置微信的Wechat.exe地址为环境变量,那么你需要传入该文件地址作为参数否则会引发错误无法打开微信！
        :param: is_maximize:\t小程序面板界面是否全屏,默认全屏。\n
        :param: wechat_maximize:\t微信主界面是否全屏,默认全屏\n
        :param: close_wechat:\t任务结束后是否关闭微信,默认关闭\n
        '''
    ...
    def open_top_stories(wechat_path:str=None,is_maximize:bool=True,wechat_maximize:bool=True,close_wechat:bool=True):
        ...
        :param: wechat_path:\t微信的WeChat.exe文件地址,当微信未登录时,该函数使用此地址启动微信并点击登录完成打开微信界面。
            这里强烈建议将微信路径加入到windows环境变量中,因为该函数默认使用windows环境变量中的Wechat.exe路径启动微信,此时调用该函数无需传入该参数
            若你没有设置微信的Wechat.exe地址为环境变量,那么你需要传入该文件地址作为参数否则会引发错误无法打开微信！
        :param: is_maximize:\t看一看界面是否全屏,默认全屏。\n
        :param: wechat_maximize:\t微信主界面是否全屏,默认全屏\n
        :param: close_wechat:\t任务结束后是否关闭微信,默认关闭\n
        '''
        main_window=Tools.open_wechat(wechat_path=wechat_path,is_maximize=wechat_maximize)
    ...
    def open_search(wechat_path:str=None,is_maximize:bool=True,wechat_maximize:bool=True,close_wechat:bool=True):
        ...
        :param: wechat_path:\t微信的WeChat.exe文件地址,当微信未登录时,该函数使用此地址启动微信并点击登录完成打开微信界面。
            这里强烈建议将微信路径加入到windows环境变量中,因为该函数默认使用windows环境变量中的Wechat.exe路径启动微信,此时调用该函数无需传入该参数
            若你没有设置微信的Wechat.exe地址为环境变量,那么你需要传入该文件地址作为参数否则会引发错误无法打开微信！
        :param: is_maximize:\t搜一搜界面是否全屏,默认全屏。\n
        :param: wechat_maximize:\t微信主界面是否全屏,默认全屏\n
        :param: close_wechat:\t任务结束后是否关闭微信,默认关闭\n
        '''
        main_window=Tools.open_wechat(wechat_path=wechat_path,is_maximize=wechat_maximize)
    ...
    def open_channels(wechat_path:str=None,is_maximize:bool=True,wechat_maximize:bool=True,close_wechat:bool=True):
        ...
        :param: wechat_path:\t微信的WeChat.exe文件地址,当微信未登录时,该函数使用此地址启动微信并点击登录完成打开微信界面。\n
            这里强烈建议将微信路径加入到windows环境变量中,因为该函数默认使用windows环境变量中的Wechat.exe路径启动微信,此时调用该函数无需传入该参数
            若你没有设置微信的Wechat.exe地址为环境变量,那么你需要传入该文件地址作为参数否则会引发错误无法打开微信！
        :param: is_maximize:\t视频号界面是否全屏,默认全屏。\n
        :param: wechat_maximize:\t微信主界面是否全屏,默认全屏\n
        :param: close_wechat:\t任务结束后是否关闭微信,默认关闭\n  
        '''
        main_window=Tools.open_wechat(wechat_path=wechat_path,is_maximize=wechat_maximize)
...
class API():
    ...
    def open_wechat_official_account(official_acount_name:str,load_delay:float=2,wechat_path:str=None,is_maximize:bool=True,close_wechat:bool=True):
        '''
        该方法用于打开指定的微信公众号\n
        :param: official_acount_name:\t微信公众号名称\n
        :param: load_delay:\t加载查询结果的时间,单位:s\n
        :param: wechat_path:\t微信的WeChat.exe文件地址,当微信未登录时,该函数使用此地址启动微信并点击登录完成打开微信界面。\n
            这里强烈建议将微信路径加入到windows环境变量中,因为该函数默认使用windows环境变量中的Wechat.exe路径启动微信,此时调用该函数无需传入该参数
            若你没有设置微信的Wechat.exe地址为环境变量,那么你需要传入该文件地址作为参数否则会引发错误无法打开微信！
        :param: is_maximize:\t微信界面是否全屏,默认全屏。\n
    ...
    def search_channels(search_content:str,load_delay:float=2,wechat_path:str=None,wechat_maximize:bool=True,is_maximize:bool=True,close_wechat:bool=True):
        '''
        该方法用于打开视频号并搜索指定内容\n
        :param: search_content:在视频号内待搜索内容\n
        :param: load_delay:加载查询结果的时间,单位:s\n
        :param: wechat_path:微信的WeChat.exe文件地址,当微信未登录时,该函数使用此地址启动微信并点击登录完成打开微信界面。\n
            这里强烈建议将微信路径加入到windows环境变量中,因为该函数默认使用windows环境变量中的Wechat.exe路径启动微信,此时调用该函数无需传入该参数
            若你没有设置微信的Wechat.exe地址为环境变量,那么你需要传入该文件地址作为参数否则会引发错误无法打开微信！
