...
函数:函数为上述模块内的所有方法\n
使用该模块内的方法时,你可以:\n
from wechatpy.wechatauto import Messages\n
Messages.send_messages_to_friend()\n
或者\n
from wechatpy.wechatauto import send_messages_to_friend\n
send_messages_to_friend()\n
或者\n
from wechatpy import wechatauto as wechat\n
...
from pywechat127.uielements import ToolBar
from pywechat127.wechatTools import Tools
from pywechat127.wechatTools import mouse
from pywechat127.wechatTools import Desktop
...
class Messages():
    ...
    @staticmethod
    def send_messages_to_friend(friend:str,messages:list[str],delay:int=2,wechat_path:str=None,is_maximize:bool=True):
        '''
        该方法用于给单个好友或群聊发送多条信息\n
        friend:好友或群聊备注。格式:friend="好友或群聊备注"\n
    ...
    @staticmethod
    def send_messages_to_firends(friends:list[str],messages:list[list[str]],wechat_path:str=None,delay:int=2,is_maximize:bool=True):
        '''
        该方法用于给多个好友或群聊发送多条信息\n
    ...
    @staticmethod
    def send_message_to_friends(friends:list[str],message:list[str],wechat_path:str=None,delay:int=2,is_maximize:bool=True):
        '''
        该方法用于给多个好友或群聊发送单条信息\n
    ...
    @staticmethod
    def forward_message(friends:list[str],message:str,wechat_path:str=None):
        '''
        该方法用于给多个好友或群聊转发单条信息\n
...
class Files():
    ...
    @staticmethod
    def send_file_to_friends(friends:list[str],file_paths:list[str],wechat_path:str=None,delay:int=2,is_maximize:bool=True,with_messages:bool=False,messages:list[list[str]]=[],message_first:bool=False):
        '''
    ...
    @staticmethod
    def send_files_to_friends(friends:list[str],folder_paths:list[str],wechat_path:str=None,delay:int=2,is_maximize:bool=True,with_messages:bool=False,messages:list[list[str]]=[],message_first:bool=False):
        '''
...
class WechatSettings():
    ...
    def convert_to_text(wechat_path:str=None,is_maximize:bool=True):
        ...
        if check_box.get_toggle_state():
            ...
            print("已开启自动语音转文字")
    @staticmethod
    def clear_chat_history(wechat_path:str=None,is_maximize:bool=True):
        '''
        wechat_path:微信的WeChat.exe文件地址,当微信未登录时,该函数使用此地址启动微信并点击登录完成打开微信界面。\n
        这里强烈建议将微信路径加入到windows环境变量中,因为该函数默认使用windows环境变量中的Wechat.exe路径启动微信,此时调用该函数无需任何参数\n
        若你没有设置微信的Wechat.exe地址为环境变量,那么你需要传入该文件地址作为参数否则会引发错误无法打开微信！\n
...
class FriendSettings():
    ...
    @staticmethod
    def tickle_friend(friend:str,times:int=1,wechat_path:str=None,is_maximize:bool=True):
    ...
    def create_group_chat(friends:list[str],group_name:str=None,wechat_path:str=None,is_maximize:bool=True,messages:list=[]):
        ...
        if messages:
            ...
            for message in message:
                ...
                pyautogui.hotkey('alt','s')
        if group_name:
            three_points=main_window.child_window(**Main_window.ChatMessage)
...
class Auto_Reply():
    ...
    @staticmethod
    def auto_reply_to_someones_message(friend:str,duration:str,content:str,wechat_path:str=None,is_maximize:bool=True):
        '''
    friend:好友或群聊备注\n
